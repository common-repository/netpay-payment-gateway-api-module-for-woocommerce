...
class Connection {
    ...
    private $user_ssl_auth = TRUE; //By default all API request need to use ssl, Download your test ssl form merchant portal
    ...
    public function set_ssl_path($ssl_path){
        if(is_array($ssl_path)){
            if(isset($ssl_path['ssl_certificate_path'])){
                $this->ssl_certificate_path = $ssl_path['ssl_certificate_path'];
            }
            if(isset($ssl_path['ssl_key_path'])){
                $this->ssl_key_path = $ssl_path['ssl_key_path'];
            }
            if(isset($ssl_path['ssl_key_password'])){
                $this->ssl_key_password = $ssl_path['ssl_key_password'];
            }
			if(isset($ssl_path['use_cert'])){
                $this->user_ssl_auth = (($ssl_path['use_cert'] === 'yes')? TRUE : FALSE);
            }
        }
    }
    ...
    public function get_error_message(){
        ...
        if ($this->error_string) {
            $str .= "<strong>Curl Error Code:</strong> " . $this->error_code . "<br/>\n";
