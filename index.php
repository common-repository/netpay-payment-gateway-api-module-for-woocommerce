   	...
   	*/
   	class WC_Tech_Netpayapi extends WC_Payment_Gateway 
   	{
      	...
      	public function __construct(){
			...
			$this->liveurl          		= 	(($this->settings['netpayapi_live_url']!='')?$this->settings['netpayapi_live_url']:'https://integration.revolution.netpay.co.uk/v1/');
			$this->testurl          		= 	(($this->settings['netpayapi_test_url']!='')?$this->settings['netpayapi_test_url']:'https://integrationtest.revolution.netpay.co.uk/v1/');
			$this->netpayapi_use_ssl     	= 	$this->settings['netpayapi_use_cert'];
			$this->netpayapi_ssl_certificate= 	$this->settings['netpayapi_cert_path'];
			$this->netpayapi_ssl_key     	= 	$this->settings['netpayapi_key_path'];
			$this->netpayapi_cert_password  = 	$this->settings['netpayapi_cert_password'];
			...
			add_action( 'woocommerce_api_wc_tech_netpayapi_3ds' , array( $this, 'process_3ds_form' ) );
		...
		// function to show fields in admin configuration form
      	function init_form_fields(){
         	$this->form_fields = array(
				...
				'netpayapi_live_url' => array(
                  	'title'        => __('Live Mode Web Service End Point', 'tech'),
                  	'type'         => 'text',
	        		'default'      => 'https://integration.revolution.netpay.co.uk/v1/',
                  	'description'  =>  __('This is uri from your service api integration tab', 'tech')),
				'netpayapi_test_url' => array(
                  	'title'        => __('Test Mode Web Service End Point', 'tech'),
                  	'type'         => 'text',
	        		'default'      => 'https://integrationtest.revolution.netpay.co.uk/v1/',
                  	'description'  =>  __('This is uri from your service api integration tab', 'tech')),
            	'netpayapi_use_cert'      => array(
                	'title'        => __('Use SSL for API connection', 'tech'),
                  	'type'         => 'select',
            	  	'options'      => array('yes'=>'Yes', 'test'=>'No'),
                  	'description'  => "Sends SSL certificate specified to NetPay servers to authenticate request" ),
				'netpayapi_cert_path' => array(
                  	'title'        => __('SSL Certificate Path', 'tech'),
                  	'type'         => 'text',
                  	'description'  =>  __('This is path to file with certificate that you want to use to authenticate connection to NetPay', 'tech')),
				'netpayapi_key_path' => array(
                  	'title'        => __('SSL Key Path', 'tech'),
                  	'type'         => 'text',
                  	'description'  =>  __('This is path to file with key that you want to use to authenticate connection to NetPay', 'tech')),
				'netpayapi_cert_password' => array(
                  	'title'        => __('SSL Certificate Password', 'tech'),
                  	'type'         => 'text',
                  	'description'  =>  __('This is password for SSL certificate, leave empty if none', 'tech')),
		...
		function retrieveTokenCardDetails($token){
			...
			$rest   = new Connection($this->gateway_url(), $this->get_headers_array(), $this->get_ssl_path());
						...
						case 'CARD_ENROLLED':
								...
								if(strtoupper($token) != 'ERROR'){ // temporary token created
									...
									if (version_compare(WOOCOMMERCE_VERSION, '2.1', '<')) {
										...
										$relay_url = add_query_arg( 'pareq',  rawurlencode ($response->ddd_secure_redirect->simple->html_body_content), $relay_url );
									} else {
										$redirect_url=$this->get_return_url( $order );
										$relay_url = add_query_arg( 'wc-api',  get_class( $this ) . '_3ds', $redirect_url );
										$relay_url = add_query_arg( 'pareq',  rawurlencode ($response->ddd_secure_redirect->simple->html_body_content), $relay_url );
									}
		...
		function process_card_enrolled(){
			...
			if(strtoupper($response->result) == 'SUCCESS'){
				if(strtoupper($response->ddd_secure->summary_status) == 'AUTHENTICATION_SUCCESSFUL'){
					...
					$errorDescription = $this->getErrorDescription($response->ddd_secure->summary_status);
		...
		function process_3ds_form(){
			$form = "<div id='npAuthFrm'>";
			$form .= base64_decode(rawurldecode ($_GET['pareq']));
			$form .= "</div>";
		...
		function get_ssl_path(){
			//CLIENT SSL PATH AND PASSWORD
			return array(
				'ssl_certificate_path' => $this->netpayapi_ssl_certificate, 
				'ssl_key_path' 		   => $this->netpayapi_ssl_key,
				'ssl_key_password' 	   => $this->netpayapi_cert_password, // if no password set empty
				'use_cert' 	   		   => $this->netpayapi_use_ssl
			);
		}
...
//			$params['ddd_secure_redirect']['goods_description']	 	= 'Payment for '.get_bloginfo('name');
		...
		public function prepare_post_array($order_id){
			...
			$params['transaction']['description'] 	= 	substr("New order with order id ".$order_id." and amount ".$currency." ".$order->order_total." has been placed.", 0, 100);
			$params['transaction']['amount'] 		= 	$order->order_total;
			$params['transaction']['currency'] 		= 	$currency;
			$params['transaction']['source'] 		= 	"INTERNET";
			...
			if( isset($paymentOption) && $paymentOption != 'new_card'){
				...
				if($this->get_post('cc_title') != ''){
					$params['payment_source']['card']['holder']['title'] 	= 	trim(substr($this->get_post('cc_title'), 0, 20));
				}
				$params['payment_source']['card']['holder']['firstname']  	= 	trim(substr($this->get_post('cc_fname'), 0, 50));
				if($this->get_post('cc_mname') != ''){
					$params['payment_source']['card']['holder']['middlename']= 	trim(substr($this->get_post('cc_mname'), 0, 50));
				}
				$params['payment_source']['card']['holder']['lastname'] 	= 	trim(substr($this->get_post('cc_lname'), 0, 50));
				$params['payment_source']['card']['holder']['fullname'] 	= 	trim(substr($this->get_post('cc_title')." ".$this->get_post('cc_fname')." ".$this->get_post('cc_mname')." ".$this->get_post('cc_lname'), 0, 100));
			}
			
			/* billing information parameters*/
			$params['billing']['bill_to_company'] 	 = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_company)), 0, 100));
			$params['billing']['bill_to_address']	 = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_address_1.' '.$order->billing_address_2)), 0, 100));
			$params['billing']['bill_to_town_city']  = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_city)), 0, 50));
			$params['billing']['bill_to_county'] 	 = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_state)), 0, 50));
			$params['billing']['bill_to_postcode'] 	 = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_postcode)), 0, 10));
			$params['billing']['bill_to_country'] 	 = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($this->getValidCountryCode($order->billing_country))), 0, 3));
			...
			foreach($woocommerce->cart->cart_contents as $cartItem){
				$params['order']['order_items'][$itemCnt]['item_id'] 		  = trim(substr(preg_replace( "/[^0-9]/", "", $cartItem['product_id'] ), 0, 25));
				$params['order']['order_items'][$itemCnt]['item_name'] 		  = trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF\|\}\{]/', '', strip_tags($cartItem['data']->post->post_name)), 0, 100));
			...
			if( trim($order->get_shipping_address()) == '' ) {
				$params['shipping']['ship_to_firstname']	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_first_name)), 0, 50));
				$params['shipping']['ship_to_lastname'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_last_name)), 0, 50));
				$params['shipping']['ship_to_fullname'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_first_name.' '.$order->billing_last_name)), 0, 100));
				$params['shipping']['ship_to_company'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_company)), 0, 100));
				$params['shipping']['ship_to_address'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_address_1.' '.$order->billing_address_2)), 0, 100));
				$params['shipping']['ship_to_town_city']	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_city)), 0, 50));
				$params['shipping']['ship_to_county'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_state)), 0, 50));
				$params['shipping']['ship_to_postcode'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_postcode)), 0, 10));
				$params['shipping']['ship_to_country'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($this->getValidCountryCode($order->billing_country))), 0, 3));
			} else {
				$params['shipping']['ship_to_firstname']	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_first_name)), 0, 50));
				$params['shipping']['ship_to_lastname'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_last_name)), 0, 50));
				$params['shipping']['ship_to_fullname'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_first_name.' '.$order->shipping_last_name)), 0, 100));
				$params['shipping']['ship_to_company'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_company)), 0, 100));
				$params['shipping']['ship_to_address'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_address_1.' '.$order->shipping_address_2)), 0, 100));
				$params['shipping']['ship_to_town_city']	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_city)), 0, 50));
				$params['shipping']['ship_to_county'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_state)), 0, 50));
				$params['shipping']['ship_to_postcode'] 	= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->shipping_postcode)), 0, 10));
				$params['shipping']['ship_to_country'] 		= trim(substr(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($this->getValidCountryCode($order->shipping_country))), 0, 3));
			}
			...
			$params['customer']['customer_ip_address'] 	= trim(substr($_SERVER['REMOTE_ADDR'], 0, 15));
			$params['customer']['customer_hostname']   	= trim(substr($_SERVER['HTTP_HOST'], 0, 60));
			$params['customer']['customer_browser'] 	= trim(substr($uagent['name'], 0, 200));
		...
		function _createCardToken($tokenType){
			...
			if($this->get_post('cc_title') != ''){
				$token_params['payment_source']['card']['holder']['title']		= 	trim(substr($this->get_post('cc_title'), 0, 20));
			}
			$token_params['payment_source']['card']['holder']['firstname'] 		= 	trim(substr($this->get_post('cc_fname'), 0, 50));
			if($this->get_post('cc_mname') != ''){
				$token_params['payment_source']['card']['holder']['middlename']	= 	trim(substr($this->get_post('cc_mname'), 0, 50));
			}
			$token_params['payment_source']['card']['holder']['lastname'] 		= 	trim(substr($this->get_post('cc_lname'), 0, 50));
			$token_params['payment_source']['card']['holder']['fullname'] 		= 	trim(substr($fullname, 0, 100));
